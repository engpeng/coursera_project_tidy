...
## Merge training and test datasets into one data set.
...
tidy.average <- ddply(melt(tidy, id.vars=c("Subject", "Activity")), .(Subject, Activity), summarise, MeanSamples=mean(value))

write.csv(tidy.average, file = "tidy.average.txt",row.names = FALSE)
